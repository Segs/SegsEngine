...
namespace eastl
{
	...
	public:
        ...
        DequeIterator(const iterator& x) EA_NOEXCEPT;
        ...
        constexpr this_type& operator=(const DequeIterator &n) EA_NOEXCEPT = default;
	...
	protected:
        T*  mpCurrent=nullptr;          // Where we currently point. Declared first because it's used most often.
        T*  mpBegin=nullptr;            // The beginning of the current subarray.
        T*  mpEnd=nullptr;              // The end of the current subarray. To consider: remove this member, as it is always equal to 'mpBegin + kDequeSubarraySize'. Given that deque subarrays usually consist of hundreds of bytes, this isn't a massive win. Also, now that we are implementing a zero-allocation new deque policy, mpEnd may in fact not be equal to 'mpBegin + kDequeSubarraySize'.
        T** mpCurrentArrayPtr=nullptr;  // Pointer to current subarray. We could alternatively implement this as a list node iterator if the deque used a linked list.
    ...
    template <typename T, typename Pointer, typename Reference, unsigned kDequeSubarraySize>
    DequeIterator<T, Pointer, Reference, kDequeSubarraySize>::DequeIterator(const iterator& x) EA_NOEXCEPT
