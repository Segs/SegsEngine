...
class basic_sparse_set {
    ...
    using sparse_container_type = eastl::vector<Entity *, Allocator>;
    using packed_container_type = eastl::vector<Entity, Allocator>;
    ...
    [[nodiscard]] auto &assure_at_least(const Entity entt) {
        ...
        if(!sparse[page]) {
            ...
            sparse[page] = (Entity *)page_allocator.allocate(entity_traits::page_size*sizeof(Entity));
    ...
    void release_sparse_pages() {
        ...
        for(auto &&page: sparse) {
            if(page != nullptr) {
                ...
                page_allocator.deallocate(page, entity_traits::page_size);
                page = nullptr;
            }
        }
    }
...
public:
    ...
    basic_sparse_set(basic_sparse_set &&other, const allocator_type &allocator) ENTT_NOEXCEPT
        : sparse{std::move(other.sparse), allocator},
          ...
          free_list{std::exchange(other.free_list, tombstone)},
          mode{other.mode} {
        ENTT_ASSERT(packed.get_allocator() == other.packed.get_allocator(), "Copying a sparse set is not allowed");
    }
