...
class basic_observer {
    ...
    void connect(basic_registry<Entity> &reg, eastl::index_sequence<Index...>) {
        static_assert(sizeof...(Matcher) < eastl::numeric_limits<payload_type>::digits);
...
public:
     ...
     */
    template<typename... Matcher>
    basic_observer(basic_registry<entity_type,Allocator> &reg, basic_collector<Matcher...>)
        : target{&reg},
          release{},
          view{}
    {
        connect<Matcher...>(reg, eastl::index_sequence_for<Matcher...>{});
    }
     ...
     */
    template<typename... Matcher>
    void connect(basic_registry<entity_type,Allocator> &reg, basic_collector<Matcher...>) {
        disconnect();
        connect<Matcher...>(reg, eastl::index_sequence_for<Matcher...>{});
        target = &reg;
        view.clear();
    ...
    iterator begin() const ENTT_NOEXCEPT {
        return view.sparse_set<entity_type,Allocator>::begin();
    }
    ...
    template<typename Func>
    void each(Func func) {
        eastl::as_const(*this).each(eastl::move(func));
        clear();
    }
