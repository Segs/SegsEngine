...
static bool compare(const void *lhs, const void *rhs) {
    if constexpr(!eastl::is_function_v<Type> && is_equality_comparable_v<Type>) {
...
struct meta_node<Type> {
    static_assert(eastl::is_same_v<Type, eastl::remove_cv_t<eastl::remove_reference_t<Type>>>);
    ...
    static meta_type_node * resolve() ENTT_NOEXCEPT {
        static meta_type_node node{
            ...
            nullptr,
            nullptr,
            eastl::is_void_v<Type>,
            eastl::is_integral_v<Type>,
            eastl::is_floating_point_v<Type>,
            eastl::is_array_v<Type>,
            eastl::is_enum_v<Type>,
            eastl::is_union_v<Type>,
            eastl::is_class_v<Type>,
            eastl::is_pointer_v<Type>,
            eastl::is_pointer_v<Type> && eastl::is_function_v<eastl::remove_pointer_t<Type>>,
            eastl::is_member_object_pointer_v<Type>,
            eastl::is_member_function_pointer_v<Type>,
            eastl::extent_v<Type>,
            &compare<Type>, // workaround for an issue with VS2017
            []() ENTT_NOEXCEPT -> meta_type_node * {
                return meta_node<eastl::remove_const_t<eastl::remove_pointer_t<Type>>>::resolve();
            },
            []() ENTT_NOEXCEPT -> meta_type_node * {
                return meta_node<eastl::remove_const_t<eastl::remove_extent_t<Type>>>::resolve();
            }
        };
...
template<typename... Type>
struct meta_info: meta_node<eastl::remove_cv_t<eastl::remove_reference_t<Type>>...> {};
...
class meta_any {
    ...
    using storage_type = eastl::aligned_storage_t<sizeof(void *), alignof(void *)>;
    ...
    template<typename Type, typename = eastl::void_t<>>
    struct type_traits {
        template<typename... Args>
        static void * instance(storage_type &storage, Args &&... args) {
            auto instance = eastl::make_unique<Type>(eastl::forward<Args>(args)...);
        ...
        static void * copy(storage_type &storage, const void *other) {
            auto instance = eastl::make_unique<Type>(*static_cast<const Type *>(other));
            new (&storage) Type *{instance.get()};
            return instance.release();
        }
    ...
    template<typename Type>
    struct type_traits<Type, eastl::enable_if_t<sizeof(Type) <= sizeof(void *) && eastl::is_nothrow_move_constructible_v<Type>>> {
        template<typename... Args>
        static void * instance(storage_type &storage, Args &&... args) {
            return new (&storage) Type{eastl::forward<Args>(args)...};
        }
        ...
        static void * steal(storage_type &to, void *from, destroy_fn_type *destroy_fn) {
            void * const instance = new (&to) Type{eastl::move(*static_cast<Type *>(from))};
            destroy_fn(from);
            return instance;
        }
    };
...
public:
    ...
    explicit meta_any(eastl::in_place_type_t<Type>, [[maybe_unused]] Args &&... args)
        : meta_any{}
    {
        node = internal::meta_info<Type>::resolve();

        if constexpr(!eastl::is_void_v<Type>) {
            using traits_type = type_traits<eastl::remove_cv_t<eastl::remove_reference_t<Type>>>;
            static_assert(eastl::is_copy_constructible_v<Type>);

            instance = traits_type::instance(storage, eastl::forward<Args>(args)...);
     ...
     */
    template<typename Type>
    explicit meta_any(eastl::reference_wrapper<Type> type)
        : meta_any{}
    {
        node = internal::meta_info<Type>::resolve();
        instance = &type.get();
     ...
     * @param type An instance of an object to use to initialize the container.
     */
    template<typename Type, typename = eastl::enable_if_t<!eastl::is_same_v<eastl::remove_cv_t<eastl::remove_reference_t<Type>>, meta_any>>>
    meta_any(Type &&type)
        : meta_any{eastl::in_place_type<eastl::remove_cv_t<eastl::remove_reference_t<Type>>>, eastl::forward<Type>(type)}
    {}
    ...
    meta_any & operator=(Type &&type) {
        return (*this = meta_any{eastl::forward<Type>(type)});
    }
    ...
    void * data() ENTT_NOEXCEPT {
        return const_cast<void *>(eastl::as_const(*this).data());
    }
    ...
    Type * try_cast() ENTT_NOEXCEPT {
        return const_cast<Type *>(eastl::as_const(*this).try_cast<Type>());
    }
    ...
    Type & cast() ENTT_NOEXCEPT {
        return const_cast<Type &>(eastl::as_const(*this).cast<Type>());
    }
    ...
    bool convert() {
        ...
        if(!valid) {
            if(auto any = eastl::as_const(*this).convert<Type>(); any) {
                swap(any, *this);
                valid = true;
            }
        }
    ...
    void emplace(Args &&... args) {
        *this = meta_any{eastl::in_place_type_t<Type>{}, eastl::forward<Args>(args)...};
    }
    ...
    friend void swap(meta_any &lhs, meta_any &rhs) ENTT_NOEXCEPT {
        if(lhs.steal_fn && rhs.steal_fn) {
            ...
            eastl::swap(rhs.instance, lhs.instance);
        } else if(rhs.steal_fn) {
            rhs.instance = rhs.steal_fn(lhs.storage, rhs.instance, rhs.destroy_fn);
            eastl::swap(rhs.instance, lhs.instance);
        } else {
            eastl::swap(lhs.instance, rhs.instance);
        }

        eastl::swap(lhs.node, rhs.node);
        eastl::swap(lhs.destroy_fn, rhs.destroy_fn);
        eastl::swap(lhs.copy_fn, rhs.copy_fn);
        eastl::swap(lhs.steal_fn, rhs.steal_fn);
    }
...
public:
     ...
     */
    template<typename Type, typename = eastl::enable_if_t<!eastl::is_same_v<eastl::remove_cv_t<eastl::remove_reference_t<Type>>, meta_handle>>>
...
struct meta_ctor {
    ...
    meta_any invoke(Args &&... args) const {
        eastl::array<meta_any, sizeof...(Args)> arguments{{eastl::forward<Args>(args)...}};
        meta_any any{};
    ...
    eastl::enable_if_t<eastl::is_invocable_v<Op, meta_prop>, void>
    prop(Op op) const ENTT_NOEXCEPT {
        internal::iterate<meta_prop>(eastl::move(op), node->prop);
    }
    ...
    meta_prop prop(meta_any key) const ENTT_NOEXCEPT {
        return internal::find_if([key = eastl::move(key)](auto *candidate) {
...
struct meta_data {
    ...
    bool set(meta_handle handle, Type &&value) const {
        return node->set(handle, meta_any{}, eastl::forward<Type>(value));
    }
    ...
    bool set(meta_handle handle, std::size_t index, Type &&value) const {
        ...
        return node->set(handle, index, eastl::forward<Type>(value));
    }
...
struct meta_func {
    ...
    meta_any invoke(meta_handle handle, Args &&... args) const {
        ...
        eastl::array<meta_any, sizeof...(Args)> arguments{{meta_handle{args}...}};
        meta_any any{};
...
class meta_type {
    ...
    auto ctor(eastl::index_sequence<Indexes...>) const ENTT_NOEXCEPT {
...
public:
     ...
     */
    template<typename Op>
    eastl::enable_if_t<eastl::is_invocable_v<Op, meta_base>, void>
    base(Op op) const ENTT_NOEXCEPT {
        internal::iterate<&internal::meta_type_node::base, meta_base>(eastl::move(op), node);
    }
    ...
    void conv(Op op) const ENTT_NOEXCEPT {
        internal::iterate<&internal::meta_type_node::conv, meta_conv>(eastl::move(op), node);
    }
    ...
    void ctor(Op op) const ENTT_NOEXCEPT {
        internal::iterate<meta_ctor>(eastl::move(op), node->ctor);
    }
    ...
    meta_ctor ctor() const ENTT_NOEXCEPT {
        return ctor<Args...>(eastl::index_sequence_for<Args...>{});
    }
     ...
     */
    template<typename Op>
    eastl::enable_if_t<eastl::is_invocable_v<Op, meta_data>, void>
    data(Op op) const ENTT_NOEXCEPT {
        internal::iterate<&internal::meta_type_node::data, meta_data>(eastl::move(op), node);
    }
     ...
     */
    template<typename Op>
    eastl::enable_if_t<eastl::is_invocable_v<Op, meta_func>, void>
    func(Op op) const ENTT_NOEXCEPT {
        internal::iterate<&internal::meta_type_node::func, meta_func>(eastl::move(op), node);
    }
    ...
    eastl::enable_if_t<eastl::is_invocable_v<Op, meta_prop>, void>
    prop(Op op) const ENTT_NOEXCEPT {
        internal::iterate<&internal::meta_type_node::prop, meta_prop>(eastl::move(op), node);
    }
     ...
     */
    meta_prop prop(meta_any key) const ENTT_NOEXCEPT {
        return internal::find_if<&internal::meta_type_node::prop>([key = eastl::move(key)](auto *candidate) {
            return candidate->key() == key;
        }, node);
    }
