...
class dense_map {
    ...
    using sparse_container_type = eastl::vector<std::size_t, Allocator>;
    using packed_container_type = eastl::vector<node_type, Allocator>;
    ...
    void move_and_pop(const std::size_t pos) {
        if(const auto last = size() - 1u; pos != last) {
            ...
            *curr = pos;
            ...
            ::new ((void *)ptr) node_type(eastl::move(packed.first().back().element));
        }
...
public:
    ...
    dense_map(const dense_map &other)
        : dense_map{other, other.get_allocator()} {}
