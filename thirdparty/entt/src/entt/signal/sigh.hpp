...
public:
    ...
    void collect(Func func, Args... args) const {
        for(auto &&call: calls) {
            if constexpr(eastl::is_void_v<Ret>) {
                if constexpr(eastl::is_invocable_r_v<bool, Func>) {
                    call(args...);
                    if(func()) { break; }
                } else {
                    call(args...);
                ...
                if constexpr(eastl::is_invocable_r_v<bool, Func, Ret>) {
...
private:
    eastl::vector<delegate<Ret(Args...)>> calls;
};
...
class sink<Ret(Args...)> {
    ...
    using difference_type = typename eastl::iterator_traits<typename decltype(signal_type::calls)::iterator>::difference_type;
...
public:
    ...
    sink before(Type *value_or_instance) {
        ...
        if(value_or_instance) {
            const auto &calls = signal->calls;
            const auto it = eastl::find_if(calls.cbegin(), calls.cend(), [value_or_instance](const auto &delegate) {
    ...
    void disconnect(Type *value_or_instance) {
        if(value_or_instance) {
            auto &calls = signal->calls;
            calls.erase(eastl::remove_if(calls.begin(), calls.end(), [value_or_instance](const auto &delegate) {
                return delegate.instance() == value_or_instance;
            }), calls.end());
        }
    }
