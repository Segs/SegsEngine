 ...
 */
template<typename Derived, typename Allocator = EASTLAllocatorType>
class emitter {
    ...
    struct event_handler: base_handler {
        ...
        using container_type = eastl::list<element_type,Allocator>;
        ...
        void clear() ENTT_NOEXCEPT override {
            if(publishing) {
                ...
                eastl::for_each(once_list.begin(), once_list.end(), func);
                eastl::for_each(on_list.begin(), on_list.end(), func);
            } else {
                once_list.clear();
                on_list.clear();
            }
        ...
        void publish(const Event &event, Derived &ref) {
            ...
            eastl::for_each(on_list.rbegin(), on_list.rend(), func);
            eastl::for_each(swap_list.rbegin(), swap_list.rend(), func);
    ...
    struct handler_data {
        eastl::unique_ptr<base_handler> handler;
    ...
    static auto type() ENTT_NOEXCEPT {
        if constexpr(is_named_type_v<Event>) {
            ...
            return handler_family::type<eastl::decay_t<Event>>;
        }
    }
    ...
    event_handler<Event> * assure() const ENTT_NOEXCEPT {
        ...
        if constexpr(is_named_type_v<Event>) {
            const auto it = eastl::find_if(handlers.begin(), handlers.end(), [htype](const auto &candidate) {
            ...
            if(hdata->handler && hdata->runtime_type != htype) {
                ...
                eastl::swap(handlers[htype], handlers.back());
                hdata = &handlers[htype];
            }
        }

        if(!hdata->handler) {
            hdata->handler = eastl::make_unique<event_handler<Event>>();
...
public:
    ...
    struct connection: private event_handler<Event>::connection_type {
        ...
        connection(typename event_handler<Event>::connection_type conn)
            : event_handler<Event>::connection_type{eastl::move(conn)}
        {}
    };
    ...
    void publish(Args &&... args) {
        assure<Event>()->publish({ eastl::forward<Args>(args)... }, *static_cast<Derived *>(this));
    }
     ...
     */
    void clear() ENTT_NOEXCEPT {
        eastl::for_each(handlers.begin(), handlers.end(), [](auto &&hdata) {
     ...
     */
    bool empty() const ENTT_NOEXCEPT {
        return eastl::all_of(handlers.cbegin(), handlers.cend(), [](auto &&hdata) {
            return !hdata.handler || hdata.handler->empty();
        });
    }

private:
    mutable eastl::vector<handler_data,Allocator> handlers{};
};
