...
void SourceView::Save( const Worker& worker, size_t start, size_t stop )
{
    ...
    assert( start < stop );
    QByteArray fname=QFileDialog::getSaveFileName(nullptr,"Save asm","","*.asm").toUtf8();
    if(fname.isEmpty())
        return;

    FILE* f = nullptr;
    const auto sz = fname.size();
    if( sz < 5 || !fname.endsWith(".asm") )
    {
        char tmp[1024];
        sprintf( tmp, "%s.asm", fname.data() );
        f = fopen( tmp, "wb" );
    }
    else
    {
        f = fopen( fname.data(), "wb" );
    }
    if( f )
    {
        char tmp[16];
