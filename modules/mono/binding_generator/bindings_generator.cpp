...
struct GeneratorContext {
    ...
    String formatTypeMapStanza(StringView fmt,int arg_idx) {
        String res(fmt);
        if(arg_idx>=0) {
            res.replace("%arg%d",arg_names[arg_idx]);
        }
        return res;
    }
};
...
void gen_cs_return_type(GeneratorContext &ctx) {
    CSTypeWrapper wrap(ctx.return_type);
...
void gen_cs_arguments(GeneratorContext& ctx) {
    // ( %argtype %arg_modifier %argname [= %argdefault], ... )\n
    ...
    for(int i=0; i<argc; ++i) {
        ...
        if(i>=ctx.arg_names.size())
            ctx.arg_names.resize(i+1);
        ctx.arg_names[i] = name;

        StringView argtype = ctx.mapper.render(wrap,CS_INTERFACE,ctx.p_type);
        StringView arg_modifier="";
...
StringView get_internal_call_name(const CSFunction *func,bool external=true) {
    //"NativeCalls.godot_icall_AcceptDialog_register_text_enter_598860a7";
    static char buffer[512];
    buffer[0] = 0;
    StringView hash_val(func_signature_hash(func));
    assert(func->enclosing_type->cs_name().size()+func->source_type->name.size()+hash_val.size()<400);
    if(external)
    ...
    strncat(buffer,"_",1);
    strncat(buffer,hash_val.data(),hash_val.size());
...
void _generate_csmethod_icalls(GeneratorContext &ctx,const CSTypeLike* p_itype) {
    ...
    for (const CSFunction * imethod : p_itype->m_functions) {
        ...
        if (imethod->source_type->is_virtual)
            continue;
        StringView im_unique_sig=get_internal_call_name(imethod,false);
        _generate_icalldef_preamble(ctx);
        _generate_icalldef_return(ctx,imethod);
        ctx.out.append(" ");
        ctx.out.append(im_unique_sig);
        _generate_icalldef_arglist(ctx,imethod);
        String im_sig;
        im_sig += "IntPtr ptr";
...
public:
    ...
    bool create_icall_files(const ReflectionData &refl, CSTypeMapper &mapper) {
        if(!create_and_cd_to_gen_dir())
            return false;
        ...
        //m_icalls_file_ctx->out.append("// *** THIS IS AUTOGENERATED FILE, ANY CHANGES WILL GET OVERWRITTEN ***\n");
        m_icalls_file_ctx->out.append_indented("using System;\n");
        m_icalls_file_ctx->out.append_indented("using System.Runtime.CompilerServices;\n\n");
        //First produce all constructors,singleton accessors.
        for(const NamespaceInterface &ns_i : refl.namespaces) {
            m_icalls_file_ctx->start_cs_namespace(ns_i.namespace_name);
            m_icalls_file_ctx->out.append_indented("internal static class NativeCalls\n");
            m_icalls_file_ctx->start_block();
            ...
            for(const String &order : ns_i.obj_type_insert_order) {
                const TypeInterface & ti(ns_i.obj_types.at(order));
                CSType *t = CSType::by_rd(&ti);
                m_icalls_file_ctx->p_type = t;

                _generate_csmethod_icalls(*m_icalls_file_ctx,t);
            }
            m_icalls_file_ctx->end_block();
            m_icalls_file_ctx->end_block();
        }
...
struct CSReflectionVisitor {
    ...
    void finalize() {
        ...
        cs_producer.create_icall_files(m_reflection_data,cs_producer.type_mapper);
