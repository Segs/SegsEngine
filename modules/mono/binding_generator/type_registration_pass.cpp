...
void TypeRegistrationPass::_apply_prefix_to_enum_constants(const TS_Enum &p_ienum, int p_prefix_length) {

    if (p_prefix_length <= 0) return;
    ...
    for (TS_Constant *curr_const : p_ienum.m_constants) {
        ...
        if (parts.size() <= curr_prefix_length) continue;
...
void TypeRegistrationPass::registerTypesPass(const TypeInterface *ti) {
    ...
    if (type->base_type && ((const TS_Type *)type->base_type)->pass == 0) {
        ...
        registerTypesPass(((TS_Type *)type->base_type)->source_type);
    }
